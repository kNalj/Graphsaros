...
class SettingsWidget(QWidget):
    ...
    def init_ui(self):
        ...
        self.setGeometry(int(0.2 * width), int(0.2 * height), 700, 500)
        ...
        for category in self.categories:
            ...
            # scroll_area = self.build_tab()
            # self.add_tab(category, scroll_area, "Dummy " + category + " tab")
...
def main():
    ...
    buffer = QcodesData("C:\\Users\\ldrmic\\Documents\\GitHub\\Graphsaros\\other\\QCoDeS_Daniel_2d1\\IVVI_PLLT_set_IVVI_Ohmic_set.dat")
    buffer.prepare_data()
    hm = Heatmap(buffer)
    ex = HeatMapSettings(hm)

    for name, tab in ex.build_general_tabs().items():
        scroll = ex.build_tab()
        ex.add_tab("General", name, scroll)
        ex.fill_tab("General", name, tab)
    for name, tab in ex.build_display_tabs().items():
        scroll = ex.build_tab()
        ex.add_tab("Display", name, scroll)
        ex.fill_tab("Display", name, tab)
    sys.exit(app.exec_())
